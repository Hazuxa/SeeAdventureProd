...
class DatabaseHelper {
  ...
  Future<Database> _database() async {
    ...
    //emulated/0/student.db, C:\flutter\student.db
  ...
  Future<int> updateStudent(int studentID, String, fullName, String username,
    ...
    return await db.update(
        'students', data, where: 'id = ?', whereArgs: ['$studentID']);
    //return await db.rawQuery("UPDATE students SET fullName = ?, username = ?, password = ? VALUES('$fullName', '$username', '$password', WHERE id = $studentID)");
  }
  ...
  Future<List<Map<String, dynamic>>> loginUser(String username,
      String password) async {
    final database = await _database();
    return await database.query(
        'students', where: 'username = ? AND password = ?',
